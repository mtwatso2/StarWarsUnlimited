...
### 3. Combine into Excel File (Optional)

Combine all cleaned CSV files into a single Excel workbook with separate sheets:

```bash
python combine_to_excel.py
```

This will create `star_wars_unlimited_price_guide.xlsx` with one sheet per set.
...
The combined Excel file contains 6 sheets, one for each set:
- Spark of Rebellion
- Shadows of the Galaxy
- Twilight of the Republic
- Jump to Lightspeed
...
Each sheet contains the same columns as the CSV files.
...
├── combine_to_excel.py # Excel combiner script
├── *_raw.csv # Raw scraped data (generated)
├── *.csv # Cleaned data files (generated)
├── star_wars_unlimited_price_guide.xlsx # Combined Excel file (generated)
└── README.md # This file
```
